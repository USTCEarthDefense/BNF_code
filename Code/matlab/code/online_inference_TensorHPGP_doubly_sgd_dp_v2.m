...
%also a Gamma dist.)
function [model, test_LL_approx, test_LL_ELBO, model_list] = online_inference_TensorHPGP_doubly_sgd_dp_v2(data, model, need_init, test_data, test_indices)
    nepoch = model.nepoch;
    ...
    for j=1:nepoch
       ...
       while (st < N)
            ...
            %b = x(end-1);
            %addelta            
            [grad, model] = ELBO_grad_mini_batch_double_dp_v2(x, data, to_sel, to_sel_e, model);
       ...
       model_list{j} = model;
       test_LL_ELBO(j) = test_ELBO(model, test_data, test_indices);
       test_LL_approx(j) = test_ll_approx(model, test_data, test_indices);
       fprintf('epoch = %d, test_ll = %e\n', j, test_LL_approx(j));
    end
    %model = decode_parameters_to_model(x, model);
end