...
class NTF_HP:
    def __init__(self, train_ind, train_y, init_config):
        ...
        with tf.variable_scope( self.GP_SCOPE_NAME):
            ...
            self.tf_log_amp_delta = tf.Variable(0, dtype=FLOAT_TYPE)


            self.tf_log_lengthscale_trig = tf.Variable(np.zeros([self.B.shape[1], 1]), dtype=FLOAT_TYPE)
            self.tf_log_amp_trig = tf.Variable(-6, dtype=FLOAT_TYPE)
        ...
        self.delta_entries = 1.0 /(  utils_funcs.kernel_cross_tf( self.X_entries, self.X_entryEvents, self.tf_log_amp_delta, self.tf_log_lengthscale_delta) + DELTA_JITTER)
        self.T_sub_y = self.tf_T1 - self.batch_entryEvent_y  # [N_e, 1]
        self.lag_effect =  ( 1.0 - tf.exp( -self.delta_entries *  tf.transpose( self.T_sub_y))) /  self.delta_entries #[ N_i, N_e]
        self.K_trig = utils_funcs.kernel_cross_tf(self.X_entries, self.X_entryEvents, self.tf_log_amp_trig, self.tf_log_lengthscale_trig)  # [S,Q]
        self.Trig_Mat = self.K_trig * self.lag_effect
        # integral term 2
        self.int_part2 = self.num_entries / self.batch_size_entry * self.num_events / self.batch_size_entryEvent * tf.reduce_sum( self.Trig_Mat)
        ...
        self.delta_eventSum = 1.0 / ( utils_funcs.kernel_cross_tf( self.X_events, self.X_inner_events, self.tf_log_amp_delta, self.tf_log_lengthscale_delta) + DELTA_JITTER)
        self.event_delay_effect = tf.exp(- self.delta_eventSum * self.event_delay)
        self.K_trig_event = utils_funcs.kernel_cross_tf( self.X_events, self.X_inner_events, self.tf_log_amp_trig, self.tf_log_lengthscale_trig)
        self.trig_mat_event = self.K_trig_event * self.event_delay_effect * self.valid_event_delay

        #Bias est of event rates
        self.event_rates = self.base_rate_events + self.num_events / self.batch_size_inner_event * tf.reduce_sum( self.trig_mat_event, axis=1, keepdims=True)
        ...
        self.train_ELBO = self.event_sum_term - self.int_part1 - self.int_part2  - self.KL_alpha  #- self.KL_delta
    ...
    def train(self, steps = 1, print_every = 100, test_every = False,
        ...
        for step in range( 1, steps + 1):
            ...
            batch_inner_event_ind, batch_inner_event_y = self.inner_event_ind_y_gnrt.draw_next(self.batch_size_inner_event)
            ...
            train_feed_dict = {self.batch_entry_ind: batch_entries_ind,
                               ...
                               self.batch_inner_event_ind: batch_inner_event_ind, self.batch_inner_event_y: batch_inner_event_y}

            _, ELBO_ret_min_step, KL_alpha,  int_part1, int_part2, event_sum_term, entry_base_rate, delta_entries, = self.sess.run(
                [self.min_step, self.train_ELBO, self.KL_alpha, self.int_part1, self.int_part2, self.event_sum_term, self.base_rate_entries,self.delta_entries],
            ...
            if step % print_every == 0:
                print(
                    "ELBO = %g, KL_alpha = %g, int_part1 = %g, int_part2 = %g, event sum =  %g, base rate max = %g, min = %g, delta max = %g, min = %g"
                    % (ELBO_ret_min_step, KL_alpha, int_part1, int_part2,event_sum_term, np.max( entry_base_rate), np.min(entry_base_rate), np.max(delta_entries), np.min( delta_entries)))
            #End min step<<=======================

            if step % print_every == 0:
                amp_alpha, amp_trig, amp_delta, U_reg,  = self.check_vars( [ self.tf_log_amp_alpha, self.tf_log_amp_trig, self.tf_log_amp_delta, self.U_kernel_mag])
                amp_alpha, amp_trig, amp_delta = np.exp( [ amp_alpha, amp_trig, amp_delta])
                print('amp_alpha = %g, amp_trig = %g, amp_delta = %g, U_mag = %g' % ( amp_alpha, amp_trig, amp_delta, U_reg))
    ...
    def create_standAlone_test_graph(self, test_ind, test_y):
        ...
        self.gp_base_rate_entries_test = utils_funcs.sample_pst_f_tf_MLE( self.tf_mu_alpha, self.Kmm_alpha, self.Knm_entries_test)
        ...
        delta_entries_test = 1.0 / ( utils_funcs.kernel_cross_tf( self.X_batch_entries_test, self.X_all_test_events_test, self.tf_log_amp_delta, self.tf_log_lengthscale_delta) + DELTA_JITTER)
        ...
        self.K_trig_term3_test = utils_funcs.kernel_cross_tf(self.X_batch_entries_test, self.X_all_test_events_test, self.tf_log_amp_trig, self.tf_log_lengthscale_trig)
        ...
        self.test_eventSum_Knm = utils_funcs.kernel_cross_tf(self.X_batch_event_test, self.tf_B, self.tf_log_amp_alpha, self.tf_log_lengthscale_alpha)
        self.gp_eventSum_base_rate = utils_funcs.sample_pst_f_tf_MLE( self.tf_mu_alpha, self.Kmm_alpha, self.test_eventSum_Knm)
        ...
        self.valid_event_delay_test = tf.cast(self.tf_batch_event_y_test > tf.transpose(self.tf_test_event_y_test), dtype=FLOAT_TYPE)
        self.event_delay_test = self.event_delay_test * self.valid_event_delay_test

        self.delta_eventSum_test = 1.0 / ( utils_funcs.kernel_cross_tf( self.X_batch_event_test, self.X_all_test_events_test, self.tf_log_amp_delta, self.tf_log_lengthscale_delta) + DELTA_JITTER)
        self.event_delay_effect_test = tf.exp(- self.delta_eventSum_test * self.event_delay_test)

        self.K_trig_event_sum_test = utils_funcs.kernel_cross_tf(self.X_batch_event_test, self.X_all_test_events_test, self.tf_log_amp_trig, self.tf_log_lengthscale_trig)
    ...
    def test(self, entries_batch_size, event_batch_size, verbose = False):
        ...
        while cur_idx < self.num_uniq_ind_test:
            ...
            batch_int_term, batch_int_term1, batch_int_term3 = self.sess.run( [self.int_term_test, self.int_term1_test, self.int_term3_test],
                                                                              feed_dict=feed_dict)
            ...
            if verbose:
                print("int terms %d ~ %d, int_term = %g, int term1 = %g, int term3 = %g" %( cur_idx, end_idx, lst_int_terms[-1], lst_term1s[-1], lst_term3s[-1]))
        ...
        while cur_idx < self.num_test_events:
            ...
            feed_dict = { self.tf_batch_event_ind_test:batch_test_event_ind,self.tf_batch_event_y_test:batch_test_event_y }
...
def test_data_set():
    ...
    for epoch in range(1, num_epoch + 1):
        ...
        print("test_log_llk = %g, int_term = %g, int_term1 = %g, int_term3 = %g,  eventsum_term = %g\n" % (
            test_log_p, int_term, int_term1, int_term3, eventSum_term))
        test_llk.append(test_log_p)
        if test_log_p > best_llk:
            best_llk = test_log_p
            best_U = model.check_vars( model.tf_U)

    jb.dump(test_llk, "../Result_Logs/%s_bias_GP_decay_rank_%d.jb" % ( arg_data_name, arg_rank))
    jb.dump( best_U, "../Result_Logs/%s_bias_GP_decay_best_U_rank_%d.jb" %( arg_data_name, arg_rank))
    utils_funcs.log_results('../Result_Logs/Bias_GP_decay.txt', arg_data_name, arg_rank, arg_lr, test_llk)
    model.sess.close()

def test_911():
    (ind, y), ( train_ind, train_y),( test_ind, test_y) = utils_funcs.load_dataSet( '911','../Data')
    ...
    for epoch in range(1, num_epoch + 1):
        ...
        print("test_log_llk = %g, int_term = %g, int_term1 = %g, int_term3 = %g,  eventsum_term = %g\n" % (
            test_log_p, int_term, int_term1, int_term3, eventSum_term))
        test_llk.append(test_log_p)
    jb.dump(test_llk, "../Result_Logs/911_bias_GP_decay_rank_%d.pkl" % arg_rank)
    utils_funcs.log_results( '../Result_Logs/Bias_GP_decay.txt', '911',arg_rank, arg_lr,test_llk )

def test_NIPS_article():

    (ind, y), (train_ind, train_y), (test_ind, test_y) = utils_funcs.load_dataSet('article', '../Data')
    nmod = ind.shape[1]
    nvec = np.max( ind, axis=0) + 1
    ...
    B = utils_funcs.init_base_gp_pseudo_inputs(U, train_ind, len_B)
    #D = utils_funcs.init_decay_gp_pseudo_inputs( U, train_ind,len_D)
    print(B.shape)
    #print(D.shape)
    ...
    for epoch in range(1, num_epoch + 1):
        ...
        test_log_p, int_term, int_term1, int_term3, eventSum_term))
        test_llk.append(test_log_p)
    jb.dump(test_llk, "../Result_Logs/%s_bias_GP_decay_rank_%d.pkl" % (arg_data_name, arg_rank))
    utils_funcs.log_results('../Result_Logs/Bias_GP_decay.txt', arg_data_name, arg_rank, arg_lr, test_llk)
...
def test_slc_crime():
    ...
    for epoch in range(1, num_epoch + 1):
        ...
        test_log_p, int_term, int_term1, int_term3, eventSum_term))
        test_llk.append(test_log_p)
    jb.dump( test_llk, "slc_bias_trig_only_GP_decay_innerBatch_size_%d.pkl" %  init_config['batch_size_inner_event'])
...
def test_chicago_crime():
    ...
    for epoch in range(1, num_epoch + 1):
        ...
        test_log_p, int_term, int_term1, int_term3, eventSum_term))
        test_llk.append(test_log_p)
    jb.dump( test_llk, "chicago_bias_trig_only_GP_decay_innerBatch_size_%d.pkl" %  init_config['batch_size_inner_event'])

if __name__ == '__main__':
    #test_911()  #-112101
    #test_NIPS_article() #-137881 if unibias, 1024:161k, 4096:139k, GP decay: 12.9
    #test_slc_crime() #-22700
    #test_chicago_crime()
